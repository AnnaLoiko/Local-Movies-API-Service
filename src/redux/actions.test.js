...
describe('ASYNC actions:', () => {
  ...
  it('should create an action getMovie BY ID', () => {
    const movieId = 1;
    mock.onGet(`http://localhost:4000/movies/${movieId}`).reply(200, {id: movieId, title: "movie title"});
    ...
    return store.dispatch(actions.getMovieById(movieId)).then(() => {
      const actions = store.getActions();
      expect(actions).toEqual(expectedAction)
    })
  });
